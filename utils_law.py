...
def bio_to_json(string, tags, index):
    item = {"string": string, "entities": []}
    entity_name = ""
    entity_start = 0
    iCount = 0
    ...
    #     if entity["type"] == "CAT":
    #         category_list.append(entity["word"])
    #     elif entity["type"] == "FEA":
    #         feature_list.append(entity["word"])
    #     elif entity["type"] == "COM":
    #         composition_list.append(entity["word"])
    #     else:
    #         craft_list.append(entity["word"])
    #
    # item["entities"] = {"标号": index, "实体": category_list, "特性": feature_list, "组分": composition_list, "工艺": craft_list}
    # print("抽取出来的实体：", item["entities"])
    ...
    # for entity in item["entities"]:
    #     if entity["type"] == "PER":
    #         ship_list.append(entity["word"])
    #     elif entity["type"] == "ORG":
    #         plane_list.append(entity["word"])
    #     elif entity["type"] == "MIS":
    #         missile_list.append(entity["word"])
    #     else:
    #         army_list.append(entity["word"])
    #
    #
    # item["entities"] = {"标号": index, "舰艇": ship_list, "飞机": plane_list, "导弹": missile_list, "战车":army_list}
    # print("抽取出来的实体：", item["entities"])
    ...
    for entity in item["entities"]:
        if entity["type"] == "clerk":
            clerk_list.append(entity["word"])
        elif entity["type"] == "judge":
            judge_list.append(entity["word"])
        elif entity["type"] == "chief_judge":
            chief_judge_list.append(entity["word"])
        elif entity["type"] == "prosecutor":
            prosecutor_list.append(entity["word"])
        elif entity["type"] == "defendant":
            defendant_list.append(entity["word"])
        elif entity["type"] == "victim":
            victim_list.append(entity["word"])
        elif entity["type"] == "witness":
            witness_list.append(entity["word"])
        elif entity["type"] == "counsel":
            counsel_list.append(entity["word"])
        elif entity["type"] == "juror":
            juror_list.append(entity["word"])
        else:
            others_list.append(entity["word"])

    item["entities"] = {"id": index, "书记员": clerk_list,
                        "审判员": judge_list, "审判长": chief_judge_list,
                        "检察员": prosecutor_list, "被告人": defendant_list,
                        "被害人": victim_list, "证人": witness_list,
                        "辩护人": counsel_list, "陪审员": juror_list,
                        "其他":others_list}
    print("抽取出来的实体：", item["entities"])
