...
def bio_to_json(string, tags, index):
    ...
    for entity in item["entities"]:
        if entity["type"] == "PER":
            ...
            location_list.append(entity["word"])
        elif entity["type"] == "POR":
            port_list.append(entity["word"])
        elif entity["type"] == "AIR":
            airport_list.append(entity["word"])
        elif entity["type"] == "BAS":
            base_list.append(entity["word"])
        elif entity["type"] == "TRO":
            troop_list.append(entity["word"])
        elif entity["type"] == "PEO":
            people_list.append(entity["word"])
        else:
            nature_list.append(entity["word"])

    item["entities"] = {"标号": index, "舰艇": ship_list,
                        "飞机": plane_list, "导弹": missile_list,
                        "战车": army_list, "地区": location_list,
                        "港口": port_list, "机场": airport_list,
                        "基地": base_list, "自然地理": nature_list,
                        "部队":troop_list,"人物":people_list}
    print("抽取出来的实体：", item["entities"])
